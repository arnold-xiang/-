...
//全局变量
HTuple hv_Information, hv_ValueList, hv_ValueList1, hv_ValueList2, hv_Length1;
int MoRow;
int MoCol;
CString s1;
CString s2;
...
BOOL CMFCUIDlg::OnInitDialog()
{
	...
	//串口通信前获取相机
	InfoFramegrabber("DirectShow", "device", &hv_Information, &hv_ValueList1);
	...
	//获取相机名称
	int isHaveCamera = 0;
	MessageBox(hv_ValueList1[0].S());

	TupleDifference(hv_ValueList2, hv_ValueList1, &hv_ValueList);
	TupleLength(hv_ValueList, &hv_Length1);
	isHaveCamera = hv_Length1[0].I();
	if (0 == isHaveCamera)
	{
		MessageBox("无法获取相机参数");
	}
	...
	strStaticSelectCamera = "选择相机";


	//规格下拉框
	m_Version.AddString(str1);
	m_Version.AddString(str2);
	m_Version.AddString(str3);
	m_Version.AddString(str4);
...
bool CMFCUIDlg::Communication()
{
	...
	//		{
	//			Sleep(30);
	//			InfoFramegrabber("DirectShow", "device", &hv_Information, &hv_ValueList2);
...
void CMFCUIDlg::OnBnClickedButton2Deal()
{
	...
	if (flag1 == true)
	{
		...
		else
		{
			...
			HTuple  hv_rowInt, hv_columnInt, hv_Column4Int;
			hv_IndexY = 10;
			hv_IndexX = 10;
			HTuple hv_LengthStopFlag;
			int StopFlag = 1;
			int a = 0;
			int b = 0;
			int c = 0;
			...
			if (strCBText == str1)
			{
				...
				TupleLength(hv_Row11, &hv_LengthStopFlag);
				StopLength = hv_LengthStopFlag[0].I();
				if (0 == StopLength)
				{
					StopFlag = 0;
					MessageBox("定位失败，无法进行识别，请重试");
				...
				else
				{
					...
					hv_Column4Int = hv_Column4.TupleInt();
					x1 = hv_Column4Int[0].I();
					hv_columnInt = hv_Column3.TupleInt();
					x2 = hv_columnInt[0].I();
					c = (x1 - x2) / 11;
					HTuple  hv_letter;
					...
					hv_letter[6] = "G";
					hv_letter[7] = "H";
					//IndexSpaceInt = (hv_Column4Int - hv_columnInt) / 9;
					hv_rowInt = hv_Row3.TupleInt();
					hv_columnInt = hv_Column3.TupleInt();
					//hv_rowyInt = IndexSpaceInt.TupleInt();
					a = hv_rowInt[0].I();
					b = hv_columnInt[0].I();
					hv_row = a;
					hv_column = b;
			...
			//白底10X10
			else if (strCBText == str3)
			{
				Rgb1ToGray(ho_Image, &ho_GrayImage1);
				GenRectangle1(&ho_Rectangle1, 0, 0, height, width);
				MinMaxGray(ho_Rectangle1, ho_GrayImage1, 0, &hv_Min, &hv_Max, &hv_Range);
				Threshold(ho_GrayImage1, &ho_Regions, hv_Min + (0.7*hv_Range), hv_Max);
				Connection(ho_Regions, &ho_ConnectedRegions1);
				SelectShape(ho_ConnectedRegions1, &ho_SelectedRegions, "area", "and", 500, 500000);
				SelectShape(ho_SelectedRegions, &ho_SelectedRegions1, "circularity", "and", 0.06835,
					0.5);
				...
				else
				{
					...
					GenRectangle1(&ho_Rectangle2, hv_Row11, hv_Column11 + (10 * hv_rowper), hv_Row11 + hv_rowper,
						hv_Column2);
					ReduceDomain(ho_GrayImage1, ho_Rectangle, &ho_ImageReduced);
					ReduceDomain(ho_GrayImage1, ho_Rectangle2, &ho_ImageReduced1);
					Threshold(ho_ImageReduced, &ho_Regions1, hv_Min + (0.7*hv_Range), hv_Max);
					SmallestCircle(ho_Regions1, &hv_Row3, &hv_Column3, &hv_Radius2);
					Threshold(ho_ImageReduced1, &ho_Regions2, hv_Min + (0.7*hv_Range), hv_Max);
					SmallestCircle(ho_Regions2, &hv_Row4, &hv_Column4, &hv_Radius3);
					...
					for (hv_IndexY = 0; hv_IndexY <= 9; hv_IndexY += 1)
					{
						for (hv_IndexX = 0; hv_IndexX <= 9; hv_IndexX += 1)
						{
							GenCircle(&ho_Circle, hv_row + (hv_rowy*hv_IndexY), hv_column + (hv_columnx*hv_IndexX),
								110);
							ReduceDomain(ho_Image, ho_Circle, &ho_ImageReduced);
							FindDataCode2d(ho_ImageReduced, &ho_SymbolXLDs1, hv_DataCodeHandle, HTuple(),
								HTuple(), &hv_ResultHandles, &hv_DecodedDataStrings1);
			...
			else if (strCBText == str4)
			{
				...
				{
					hv_rowper = (hv_Column2 - hv_Column11) / 8.0;
					hv_Columnper = (hv_Column2 - hv_Column11) / 8.0;
					GenRectangle1(&ho_Rectangle, hv_Row11, hv_Column11, hv_Row11 + hv_rowper, hv_Column11 + hv_Columnper);
					GenRectangle1(&ho_Rectangle2, hv_Row11, hv_Column2 - hv_Columnper, hv_Row11 + hv_rowper,
						hv_Column2);
					ReduceDomain(ho_ImageScaled1, ho_Rectangle, &ho_ImageReduced);
					ReduceDomain(ho_ImageScaled1, ho_Rectangle2, &ho_ImageReduced1);
					Threshold(ho_ImageReduced, &ho_Regions1, hv_Min + (0.7*hv_Range), hv_Max);
					...
					hv_rowy = c;
					hv_columnx = c;
					for (hv_IndexY = 0; hv_IndexY <= 5; hv_IndexY += 1)
					{
						for (hv_IndexX = 0; hv_IndexX <= 7; hv_IndexX += 1)
						{
							GenCircle(&ho_Circle, hv_row + (hv_rowy * hv_IndexY), hv_column + (hv_columnx * hv_IndexX), 110);
							ReduceDomain(ho_Image, ho_Circle, &ho_ImageReduced);
							FindDataCode2d(ho_ImageReduced, &ho_SymbolXLDs1, hv_DataCodeHandle, HTuple(),
							...
							if (0 != (hv_Length1 == 1))
							{
								hv_Data[(8 * hv_IndexY) + hv_IndexX] = hv_DecodedDataStrings1;
							}
							TupleConcat(hv_index, hv_DecodedDataStrings1, &hv_index);
							TupleLength(hv_index, &hv_Length);
							SetTposition(m_htWindow, (hv_row - 100) + (hv_rowy * hv_IndexY), (hv_column + 10) + (hv_columnx * hv_IndexX));
							WriteString(m_htWindow, (hv_IndexX + (hv_IndexY * 8)) + 1);
						}
					}
...
void CMFCUIDlg::OnBnClickedButtonEnglish()
{
	...
	const char* HeadEnglish[] = { "Order" ,"Index","BarcodeContent","SampleContent","BoxNumber","SampleType","BoxDesc","BoxLocation","SaveTime" };
		...
		GetPrivateProfileString("Language Info", "StaticSelectCamera", "Select Camera", strStaticSelectCamera.GetBuffer(MAX_PATH), MAX_PATH, "D:\\系统文件\\EnglishConfig.ini");
		GetDlgItem(IDC_STATIC_Camera)->SetWindowText(strStaticSelectCamera);
...
void CMFCUIDlg::OnBnClickedButtonChinese()
{
	...
	GetDlgItem(IDC_BUTTON_ChangeContext)->SetWindowText(strButtonChange);
	GetPrivateProfileString("Language Info", "StaticSelectCamera", "选择相机", strStaticSelectCamera.GetBuffer(MAX_PATH), MAX_PATH, "D:\\系统文件\\ChineseConfig.ini");
	GetDlgItem(IDC_STATIC_Camera)->SetWindowText(strStaticSelectCamera);
